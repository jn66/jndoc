...
* 计算机就是一个由人来控制的机器
* 我们要学习的语言就是人和计算机交流的工具，人通过语言控制，操作计算机。
* 编程语言和我们说中文、英文本质上没有区别，只是语法特殊。
* 语言的发展：1纸带机 机器语言 2 汇编语言 符合语言 3 现代语言 高级语言
...
call()  
apply()  
 **这两个方法都是函数对象的方法需要通过函数对象(函数名 不要())来调用**  
...
**arguments**  
 ...
 arguments是一个类数组元素，它用来封装函数执行过程中的实参 , arguments instanceof Array 是false， 不是数组
 Array.isArray(arguments); 也是false
	所以即使不定义形参，也可以通过arguments来使用实参  
	调用函数的时候，我们所传递的实参都会做arguments中保存
 **arguments中有一个属性callee表示当前执行的函数对象**  arguments.callee  就是这个当前的函数的对象
...
通过构造函数创建出来的对象，上面有__proto__属性，指向构造函数的原型。
原型上也有__proto__ 指向原型的原型 
...
使用构造函数创建，也可以添加元素，作为参数引入，逗号隔开
```javascript
 var arr2 = new Array(10,20,30);  
```

创建一个长度为10的 ，不一定符合要求，只想创建一个10的
...
两个对象的3 不相等， 包装对象的false 是对象，if（）转换成布尔也是true
```javascript
...
**创建正则表达式**  
 ...
 var reg = /正则表达式/匹配模式 （匹配模式可以多个一起写：/gi）    i忽略大小写
...
**量词**  
	 ...
	 (ab){3}  ab连续出现3次
	 {n} 正好出现n次  
	 {m,n} 出现mn次  ｛1,3｝出现1次到3次
	 {m,} m次以上  

+至少一个，相当于{1,}   /ab+c/ 至少一个b
*0个或多个，相当于{0,}  
 ? 0个或1个，相当于{0,1}	  
...
**边界表达式**（不要在java中用，javaScript中用）  
	^:正则开始  
	$:正则结束 ：**注意结束前一个才是结束匹配**  
	如果做正则表达式中同时使用^ $则要求字符串必须完全符合正则表达式
	reg/^a|a$/ 以a开头或者以a结尾
...
replace（） 字符串指定内容替换为新的内容，写上g是全局，不写就是第一个
去掉两端的空格:  
...
str = "1a2b3c4d"
split() 可以将一个字符串拆分数字，可以传递一个正则表达式。 即使不指定全局g，也会全局
str.split(/[A-z]/) 根据英文字母拆

str.match(/[A-z]/g) 匹配全部的英文字母并返回
可以根据正则表达式，从一个字符串符合的条件中提取是来，默认情况下我们的match只会找到第一个符合要求的内容，找到后就停止搜索，我们可以设置正则表达式为全局模式，这样就会匹配到所有的内容，可以为一个正则表达式设置多个匹配模式，且顺序无所谓。match会将匹配到的内容封装到数组中，即使只有一个结果。

search() 搜索字符串是否有指定内容，返回第一出现的索引，也可以传正则，没搜到返回-1。 只会查找第一个，不能g全局

字面量不用 reg = /\./
做构造函数时，\表示转移，如果想\ 需要两个 reg = /\\/
# DOM  
...
可以为按钮的对应事件绑定处理函数的形式响应事件, 事件被触发时, 其对应的函数将会被调用.
```javascript  
<body>  
		<button id="btn">按钮</button>  
...
childNodes **属性** 会获取包括文本节点在内的所有节点  
...
元素.children   属性
 获取当前元素的**所有子元素**   不包括空白文本了 yeah 所有浏览器都兼容

元素.firstChild   属性
 获取当前元素的**第一个子节点**，会获取到空白的文本子节点  
 
 fristElementChild 第一个元素节点 不支持IE8 
...
元素.firstChild.nodeValue   文本节点的nodeValue就是文本内容
...
myClick("btn07",function(){  
	...
	*/  
    ...
    //向li中设置文本   不然就得设置文本节点，追加到li中
    li.innerHTML = "广州";  
	
	//city.innerHTML += "<li>广州</li>" 动静大，全部替换了 动作没了绑定的
    //将li添加到city中  ， 折中的方式。 只影响自己。
    city.appendChild(li);  
...
### 读取元素的当前样式  
通过style属性读取和设置都是内联样式， 无法读取样式表中的样式。
box1.style.width 读取行内样式
 ...
 **使用getComputedStyle()**  
 ...
 通过该方法读取到样式都是只读的不能修改  
 这个宽度获取的是实际值，如1350px，而下面那个是auto
 ...
 **使用currentStyle**  
 ...
 通过这个属性读取到的样式是只读的不能修改  读取正在显示的样式， 如果没设置，就获取默认值
...
clientHeight  
 元素的可见高度，包括元素的内容区和内边距的高度， 不带px，返回数字，直接计算。 包括padding，不包括border  、只读的，不能修改
clientWidth  
 元素的可见宽度，包括元素的内容区和内边距的宽度  只读的，不能修改
offsetHeight  
...
st = document.body.scrollTop || D ocument.documentElement.scrollTop
box1.style.left = left + "px";
box1.style.top = top + st + "px";


  pageX和pageY 可以获取鼠标相对于当前页面的坐标， 包括了滚动的距离  
	但是这个两个属性在IE8中不支持，所以如果需要兼容IE8，则不要使用  
var left = event.clientX;  
var　top = event.clientY;  

做IE8中，响应函数触发的时候，浏览器不会传递事件对象，做IE8以及以下中，
将时间对象作为windows对象的属性  window.event
...
## 事件的委派  
为每一个超链接绑定响应函数，得给每个绑定，操作麻烦，而且只能给已经有的绑定，新添加的必须重新绑定。
 ...
 **target** : event中的target表示的触发事件的对象  ，可以通过target获取到，然后判断是否有cless或者别的信息，进行下一步操作
...
局限
	使用 对象.事件 = 函数 的形式绑定函数，只能同时为一个元素的一个事件绑定一个响应函数，
	如果绑定多个，则后面不会覆盖前面的。
...
function drag(obj){  
    ...
    obj.onmousedown = function(event){  
        ...
        //设置box1捕获所有鼠标按下的事件  
        /*  
					 * setCapture()   当调用一个元素的setCaptuer()方法以后，这个元素将会把下一次所有鼠标按下的相关事件捕获到自身
        ...
        //为document绑定一个鼠标松开事件  box1上不行，重叠松手就不行了 
...
## Navigator  
 ...
 代表的当前浏览器的信息，通过该对象可以来识别不同的浏览器  
 由于历史原因，Navigator对象中的大部分属性都已经不能帮助我们识别浏览器了   效果好 Netscape  效果不好 ie, 所以ie 11 也变成Netscape 包装一下不像ie
...
如果通过uaerAgent不能判断,还可以通过一些浏览器中特有的对象,来判断浏览器的信息 比如ActiveXObject  window.ActiveXObject 
```javascript  
...
alert("你是IE浏览器~~~");  
}else if("ActiveXObject" in window){  //ie 11 不让转不二
alert("你是IE11，枪毙了你~~~");  
}  
```
...
reload()  
 ...
 如果在方法中传递一个true，作为参数，则会强制清空缓存刷新页面     = F5
...
定时器每次点击一下 就会开启一个。开启新的定时器，关闭上一个。
