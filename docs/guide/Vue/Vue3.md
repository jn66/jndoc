...
>
> 2. `reactive`重新分配一个新对象，会**失去**响应式（可以使用**`Object.assign`**去整体替换，批量的替换）。
...
2. 若该属性值是**依然**是【对象类型】，可直接编，也可写成函数，建议写成函数。只写数值，只监视每个的变化，写函数，监视整体的变化。需要写函数，然后开启深度监视，就分别的和整体的都监视。
...
> //写persons后面不行，需要些到reactive后面传函数的泛型
...
2.  接收参数：
    如果从 route 解构出变量，是从一个响应式变成普通，在页面上无法使用，需要 toRefs
...
1. 第一种修改方式，直接修改,历史线多次

   ```ts
   countStore.sum = 666;
   ```

2. 第二种修改方式：批量修改，历史线一次
...
如果一个 ref 对象在 reactive 里面，使用的时候可以不用.value
...
defineProps(["car", "getToy"]); //不用接受，可以在模板中使用
//也可以接受，然后变成函数
...
5. 如果`value`可以更换，那么就可以在组件标签上多次使用`v-model`，内部可以用两个 input
   ...
   对于原生事件，$event 就是事件对象，可以

   对于自定义事件，$event 就是触发事件时候，传递的数据。 不能。
...
父组件： v-bind 会拆解开:b="b" :c="c"， 孙给祖传递祖给孙传递函数
...
子组件：如果不 props 接收，就会全出现在$attrs 上
